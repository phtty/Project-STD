...
/**
 * @brief 控制/查询命令专用头部（8 字节时间戳，毫秒精度）
...
extern app_ldi_ctx_t g_ldi_ctx; /**< LDI 协议上下文实例 */

/** @brief 初始化 LDI 协议上下文（加载配置并填充全部状态字段）
 *  @param[out] self 待初始化的协议上下文；本函数写入其状态字段 */
...
/** @brief LDI 定时任务入口：周期发送 0EH 验证申请与 0CH 状态上报 */
 ...
 * @param[out] aux       输出：命令码，READY 时供协议任务分派查表
 ...
 *  @return 模块序号；未配置时返回 0xFF */
uint8_t app_ldi_get_device_idx(app_ldi_device_t device_type);

/** @brief 按设备类型写本机配置的模块序号
 *  @param device_type  设备类型
 *  @param device_index 模块序号 */
void app_ldi_set_device_idx(app_ldi_device_t device_type, uint8_t device_index);

/* ---- 响应帧头部构建（从内部状态，不从请求拷贝）---- */

/** @brief 构建标准响应头（4 字节时间戳，lane_code/cert 取自本机配置）
 *  @param[out] head 待填充的响应头；本函数写入其全部字段
 *  @param cmd_type  响应命令码 */
void app_ldi_build_rsp_head(app_ldi_req_head_t *head, uint8_t cmd_type);

/** @brief 构建控制/查询响应头（8 字节毫秒时间戳）
 *  @param[out] head 待填充的控制查询响应头；本函数写入其全部字段
 *  @param cmd_type  响应命令码 */
