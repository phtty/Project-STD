...
/**
 * @brief 电动栏杆机 (E4H) 参数配置 payload — 共 11 字节
 *
...
/**
 * @brief 信息显示屏 (E6H) 参数配置 payload — 共 11 字节
 *
...
/**
 * @brief 四类信号设备共用的 0BH 参数配置 payload
 *
...
/**
 * @brief LED情报板 (E9H) 参数配置 payload — 共 10 字节
 *
 *    [0] 保留    |  [1..10] Vendor
 *     1 byte     |     10 bytes
 */
...
/** @brief 语音播报设备 (F4H) 的参数配置 payload（与 LED 情报板同构） */
...
/**
 * @brief 设备重启 (0DH) 各设备通用 payload — 共 10 字节
 *
...
/**
 * @brief 电动栏杆机 (E4H) 的 1BH 栏杆控制 payload
 *
...
/**
 * @brief 信息显示屏 (E6H) 控制/查询 payload — 定长 4 字节 + 变长 text
 *
...
/**
 * @brief 通行信号灯 (E7H) 的 1BH 显示控制 payload
 *
...
/**
 * @brief 报警器 (E8H) 报警控制 (01H) payload — 共 3 字节 (不含 DeviceFuncType)
 *
...
/**
 * @brief LED情报板 (E9H) 控制/查询 payload — 定长 5 字节 + 变长 text
 *
...
/**
 * @brief 雨棚信号灯 (EAH) 的 1BH 显示控制 payload
 *
...
/**
 * @brief 雾灯 (EBH) 显示控制 (01H) payload — 共 2 字节 (不含 DeviceFuncType)
 *
 *   [0] Status | [1] WorkMode
 *     1 byte   |    1 byte
 */
...
/**
 * @brief 语音播报设备 (F4H) 播放提示音 (01H) payload — 定长 4 字节 + 变长 data
 *
...
/** @brief 设备状态信息（0CH 上报帧中每个功能模块 17 字节） */
typedef struct [[gnu::packed]] {
...
/** @brief 设备验证申请帧 DATA 域（0EH） */
typedef struct [[gnu::packed]] {
    app_ldi_req_head_t head; /**< 请求头（20 字节） */
...
/** @brief 设备状态监控上报帧 DATA 域（0CH） */
typedef struct [[gnu::packed]] {
    app_ldi_req_head_t head; /**< 上报头（20 字节） */
