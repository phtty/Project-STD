...
proto_probe_sta_t rls_probe_frame(const channel_t *ch, const ring_buffer_t *buff, uint32_t *total_len, uint8_t *aux)
{
    ...
    if (avail < sizeof(rls_frame_t) + 3) /* 最小合法帧 9B（无数据帧，如裸 AQUIRY_IP） */
        return PROTO_PROBE_WAIT;

    static uint8_t mem_pool[525] = {0};
    if (avail > sizeof(mem_pool)) /* rb 容量大于 mem_pool，先钳制防越界 */
        avail = sizeof(mem_pool);
...
[[maybe_unused]] static void rls_module_init(void)
{
    ...
    app_proto_bind_channel(s_rls_mask, CH_ID_RS232);
    app_proto_bind_channel(s_rls_mask, CH_ID_UDP);
