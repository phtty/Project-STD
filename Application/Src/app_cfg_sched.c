...
typedef struct {
    app_cfg_sched_desc_t desc;
} app_cfg_sched_owner_t;

static app_cfg_sched_owner_t s_owner_table[CFG_REGION_MAX_BLOCKS];
static uint8_t s_owner_cnt;
static dev_storage_t *s_storage_dev;
static bool s_storage_ready;

/* 所有者 → 块号（0xFF = 尚未落位）。由启动扫描按记录头里的 name 反查得到，
   不再等于注册序号 —— 见文件末尾 _cfg_sched_scan 的说明。 */
static uint8_t s_owner_block_map[CFG_REGION_MAX_BLOCKS];
static uint8_t s_scanned_owners = 0xFF; /* 扫描时的所有者数；数量变了则重扫 */

/* 调度器内部组包缓冲: 最大记录 (render ≈ 1.4KB) + 24B 头 */
static uint8_t s_scratch_buf[DEV_CFG_RECORD_HDR_SIZE + DEV_CFG_RECORD_MAX_IMAGE];

/* ---- 注册 ---- */
uint8_t app_cfg_sched_register(const app_cfg_sched_desc_t *desc)
{
    if (!desc || !desc->name || strlen(desc->name) == 0
     || strlen(desc->name) >= DEV_CFG_RECORD_NAME_MAX)
        return 0xFF;
    ...
    for (uint8_t i = 0; i < s_owner_cnt; i++)
        if (strncmp(s_owner_table[i].desc.name, desc->name, DEV_CFG_RECORD_NAME_MAX) == 0)
            return 0xFF;

    uint8_t id        = s_owner_cnt;
    s_owner_table[id].desc = *desc;
    s_owner_cnt++;
    return id;
}
...
static void _cfg_sched_bind(void)
{
    ...
    s_storage_dev = dev_w25qxx_get();

    uint32_t cap = s_storage_dev ? dev_storage_capacity(s_storage_dev) : 0;
    ...
    s_storage_ready = (cap >= BOARD_FONT_LIB_TOTAL_BYTES + CFG_REGION_BYTES);
    ...
    printf("[cfg] W25Qxx 容量 %u KB, 字库 %u KB + 配置区 %u KB -> %s\n", (unsigned)(cap / 1024U),
           ...
           s_storage_ready ? "就绪" : "不可用(持久化已禁用)");
}

/* 调度器互斥：s_scratch_buf（本文件）与 dev_cfg_record.c 的去重读回缓冲都是文件级静态，
   ...
   顺带也串行化了 _cfg_sched_scan（它写 s_owner_block_map[] / s_scanned_owners）。
...
{
    return dev_storage_capacity(s_storage_dev) - (uint32_t)(blk + 1) * CFG_REGION_SECTOR;
}
 ...
 * 让记录搬家（正式读取时 dev_cfg_record_load 会在找到的地址上做全量校验）。
 */
static void _cfg_sched_scan(void)
{
    ...
    for (uint8_t i = 0; i < CFG_REGION_MAX_BLOCKS; i++)
        s_owner_block_map[i] = 0xFF;

    /* 1) 认领：记录头里的 name 与本所有者的注册名相符 */
    for (uint8_t blk = 0; blk < CFG_REGION_MAX_BLOCKS; blk++) {
        dev_cfg_record_hdr_t hdr;
        if (dev_storage_read(s_storage_dev, _block_addr(blk), (uint8_t *)&hdr, sizeof(hdr)) != 0)
            continue;
        if ((uint8_t)hdr.name[0] == 0xFFU)
            continue; /* 空块 */

        for (uint8_t id = 0; id < s_owner_cnt; id++) {
            if (s_owner_block_map[id] != 0xFF)
                continue; /* 已被更靠前的块认领 */
            /* name 为 NUL 填充定长字段；正常写入的记录两处比较等价 */
            if (strncmp(hdr.name, s_owner_table[id].desc.name, DEV_CFG_RECORD_NAME_MAX) == 0) {
                s_owner_block_map[id] = blk;
                taken[blk]        = true;
                break;
            }
        }
...
{
    _cfg_sched_bind();
    if (!s_storage_dev || !s_storage_ready) return;
...
{
    if (!s_storage_dev || !s_storage_ready || id >= s_owner_cnt)
        return 0;

    uint8_t blk = s_owner_block_map[id];
    if (blk == 0xFF)
        return 0; /* 无空块可落位 */
...
/* ---- 加载/保存 (内部用注册名 + version + dev_cfg_record 全量校验) ---- */
dev_cfg_record_state_t app_cfg_sched_load(uint8_t id, uint8_t *payload, uint16_t payload_cap, uint16_t *payload_len)
{
    ...
    if (id >= s_owner_cnt || !s_storage_dev || !s_storage_ready) {
        if (s_lock) osMutexRelease(s_lock);
        return DEV_CFG_RECORD_STATE_IO_ERR;
    }
    ...
    dev_cfg_record_state_t sta =
        dev_cfg_record_load(s_storage_dev, addr, s_owner_table[id].desc.name, s_owner_table[id].desc.version, NULL,
...
int32_t app_cfg_sched_save(uint8_t id, const uint8_t *payload, uint16_t payload_len)
{
    ...
    sta = dev_cfg_record_save(s_storage_dev, addr, s_owner_table[id].desc.name, s_owner_table[id].desc.version, NULL,
                          payload, payload_len, s_scratch_buf, sizeof(s_scratch_buf));
    if (s_lock) osMutexRelease(s_lock);
    return sta;
...
void app_cfg_sched_load_all(void)
{
    ...
    for (uint8_t o = 0; o < s_owner_cnt; o++)
        if (s_owner_table[o].desc.load)
            s_owner_table[o].desc.load();
}
