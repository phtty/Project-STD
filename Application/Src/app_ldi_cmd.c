...
static void onbon_vms_ctrl(const ldi_ctrl_vms_t *ctrl, uint16_t mod_len)
{
    if (ctrl->device_func_type == 0x01) {
        ...
        /* 颜色映射: LDI 01H=绿 02H=红 03H=黄 */
        static const uint8_t color_map[] = {0, ONBON_COLOR_GREEN, ONBON_COLOR_RED, ONBON_COLOR_YELLOW};
        if (ctrl->font_color < sizeof(color_map))
            ctx.color = color_map[ctrl->font_color];

        /* 对齐映射: LDI format 01H=居中 02H=左 03H=右 */
        static const uint8_t align_map[] = {0, ONBON_ALIGN_H_CENTER, ONBON_ALIGN_H_LEFT, ONBON_ALIGN_H_RIGHT};
        if (ctrl->format < sizeof(align_map))
            ctx.h_align = align_map[ctrl->format];

        /* text 长度由 mod_len 推算，'_' → '\n' */
        uint16_t text_len = mod_len - sizeof(ldi_module_head_t) - sizeof(ldi_ctrl_vms_t);

        /* 字体自适应，优先选更大的字体 */
        switch (ctrl->font_size) {
            case 1:
                ctx.font_size = 16;
                break;
            case 2:
            ...
            default:
                for (int8_t i = 3; i > 0; i--) {
                    if (text_len * ((16 + (i - 1) * 8) / 2) <= 192) {
                        ctx.font_size = 16 + (i - 1) * 8;
                        break;
                    }
                }
                break;
        ...
        /* 显示时长: LDI 0=永久 → Onbon RUN_LOOP;
         *          其他 → RUN_LOOP (循环), timeout=keep_time */
        if (ctrl->keep_time == 0) {
            // ctx.run_mode = ONBON_RUN_HOLD;
            ctx.run_mode = ONBON_RUN_LOOP;
        } else {
            ctx.run_mode = ONBON_RUN_TIMEOUT;
            ctx.timeout  = ctrl->keep_time;
        }

        char gbk_text[256];
        uint8_t t = 0;
        for (uint16_t k = 0; k < text_len && t < sizeof(gbk_text) - 1; k++) {
            if (ctrl->text[k] == '_')
                gbk_text[t++] = '\n';
            else
                gbk_text[t++] = (char)ctrl->text[k];
        }
        gbk_text[t] = '\0';

        onbon_send_text(&ctx, gbk_text);

    } else if (ctrl->device_func_type == 0x02) {
        onbon_ctx_t ctx;
        onbon_ctx_init(&ctx);
        onbon_send_text(&ctx, " "); /* 清屏 */
    }
}
