...
void app_render_string(dev_display_t *display, dev_storage_t *font,
                       uint16_t x, uint16_t y,
                       const char *text, uint16_t len,
                       hub75_color_t color, font_size_t size)
{
    (void)display; (void)font; (void)x; (void)y;
    (void)text; (void)len; (void)color; (void)size;
...
uint16_t app_render_auto_x(uint16_t len, font_size_t size, text_align_t align)
{
    ...
    switch (align) {
        case ALIGN_CENTER: return (dev_display_get()->screen_rows - width) / 2;
        case ALIGN_RIGHT:  return dev_display_get()->screen_rows - width;
        default:           return 0;
    }
}
