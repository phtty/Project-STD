 ...
 *
 * 本表与 3833024 那份内容相同（同为标准 RMII 脚位），但**这是两份独立的板级事实**，
 * 不是同一份数据的复制 —— 哪块板改了接线就只改它自己的那份。
 *
 * 引脚号在 CubeMX 的 main.h 里没有标签（本工程不再重新生成 CubeMX 代码），
 * 故此处直接写端口与引脚号。注：本板的 main.h 里有一个 `ETH_STATUS`（PG5）标签，
 * 那是 CubeMX 留下的普通 GPIO 输入，与 RMII 无关，且全工程无人读取。
 */
