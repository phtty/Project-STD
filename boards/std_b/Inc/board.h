...
/* ---- 向量表偏移 ----
 * 必须与 boards/std_b/board.ld 的 FLASH_ORIGIN 保持一致：std_b 直烧 0x08000000，
...
/* ---- 显示子系统的定时器角色 ----
 * std_b：TIM2 出行同步中断（行扫描节拍），TIM3 出 8 级亮度 PWM。
 * 注意与 std_a 不同（那边是 TIM3/TIM4）——std_b 的 CubeMX 配置里没有 TIM4。
