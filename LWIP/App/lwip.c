...
/* USER CODE BEGIN 1 */
/* MQTT 状态存根（原 mqtt_app.c 中定义，待 MQTT 模块迁移后删除） */
enum { no_connect, connecting, connected } mqtt_state;
osSemaphoreId_t mqttConnSemHandle;
/* USER CODE END 1 */
...
void MX_LWIP_Init(void)
{
    ...
    netif_set_status_callback(&gnetif, ethernet_link_status_updated); // ����ͬһ���ص�����
...
static void ethernet_link_status_updated(struct netif *netif)
{
    if (netif_is_up(netif) && netif_is_link_up(netif)) {
        ...
        osSemaphoreRelease(mqttConnSemHandle); // �ͷ��ź���֪ͨmqtt���ӹ�������
        /* USER CODE END 6 */
    }
}
