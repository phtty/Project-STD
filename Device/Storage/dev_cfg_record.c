/**
 * @file    dev_cfg_record.c
 * @brief   配置记录通用服务实现（见 dev_cfg_record.h）
 */
 ...
 * DEV_CFG_RECORD_MAX_IMAGE 静态分配一个 2.6KB 的缓冲，而本函数只需要"是否一致"
 * 这个判断，逐块比即可，一旦不同立刻返回。
 *
 * 代价是多几次 read。但 save 路径本来就要擦扇区（几十到几百毫秒），
 * 多几十次几十微秒的读可以忽略；而且内容一致时反而省掉了擦写（去重的意义所在）。
 *
 * 副作用：本文件不再有文件级可变状态，dev_cfg_record_save / dev_cfg_record_load
 * 因此**都是可重入的**。 */
#define DEV_CFG_RECORD_CMP_CHUNK (64U)
...
static int32_t _same_as_stored(dev_storage_t *stor, uint32_t addr, const uint8_t *image,
                               uint16_t total)
{
    uint8_t chunk[DEV_CFG_RECORD_CMP_CHUNK];
...
static void _pad_name(char out[DEV_CFG_RECORD_NAME_MAX], const char *name)
{
    memset(out, 0, DEV_CFG_RECORD_NAME_MAX);
    if (name)
        strncpy(out, name, DEV_CFG_RECORD_NAME_MAX - 1);
}
                              ...
                              uint8_t *payload, uint16_t payload_cap, uint16_t *payload_len)
{
    ...
    char want[DEV_CFG_RECORD_NAME_MAX];

    if (!stor) return DEV_CFG_RECORD_STATE_IO_ERR;
    ...
    /* 归属 + 版本 + 长度逐项校验 */
    _pad_name(want, name);
    if (memcmp(hdr.name, want, DEV_CFG_RECORD_NAME_MAX) != 0)
        return DEV_CFG_RECORD_STATE_INVALID;
    if (hdr.version != version)
        return DEV_CFG_RECORD_STATE_INVALID;
    if (hdr.len == 0U || hdr.len > payload_cap)
        return DEV_CFG_RECORD_STATE_INVALID;

    if (dev_storage_read(stor, addr + DEV_CFG_RECORD_HDR_SIZE, payload, hdr.len) != 0)
        return DEV_CFG_RECORD_STATE_IO_ERR;

    /* payload CRC 校验 */
    dev_cfg_record_crc_fn_t crc_fn = crc ? crc : crc32_calc;
...
{
    dev_cfg_record_hdr_t *hdr = (dev_cfg_record_hdr_t *)scratch;
    uint16_t total;
    ...
    uint32_t total32 = DEV_CFG_RECORD_HDR_SIZE + (uint32_t)payload_len;
    ...
    memcpy(scratch + DEV_CFG_RECORD_HDR_SIZE, payload, payload_len);
