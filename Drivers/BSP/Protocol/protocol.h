...
// 协议探测函数类型
typedef proto_probe_sta_t (*proto_probe_fn_t)(const ch_meta_t *, const RingBuff_t *, uint32_t *, uint8_t *);

// 全局分组缓冲区指针数组
extern RingBuff_t *g_group_ringbuf[RB_GROUP_COUNT];
// 协议 → 分组映射表（每个协议属于哪个分组）
...
extern osMutexId_t g_ringbuf_mutex;
